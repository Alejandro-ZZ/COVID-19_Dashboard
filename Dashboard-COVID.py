...
def load_csv_local_data(address):
    '''
    Inputs
      address -> String that contains name of the CSV file

    Output
    A tuple with three items. The first one is a DataFrame from pandas wich
    contains information from CSV file with some cleaning processes. Second is 
    a ndarray with all departments in CSV file. Thirt is another ndarray 
    with all towns in CSV file.
    '''
    print("Reading data...")
    ...
    #'f' and 'm' values are replaced to F and M in 'Sexo' column
    ...
    #nan values are replaced to specific names in 'Ubicación del caso' and 'Estado' columns
    ...
    data = data.replace({'Estado': {'moderado': 'Moderado'}})
...
def reporte_datos(info, report, label, acum = False):
...
def get_summary(info, key_list, dpto_info = True):
    ...
    for key in key_list:
        ...
        if not isinstance(info.loc[key], pd.Series):
            ...
            casos_diag = reporte_datos(info.loc[key], 'fecha reporte web', 'Casos', True).max()[0]

            #Take cases based on their case location
            d = reporte_datos(info.loc[key], 'Ubicación del caso', 'Casos')
            ...
            d = reporte_datos(info.loc[key], 'Recuperado', 'Casos')
...
def get_info(csv_data, key_list, dpto_summary = True):
    ...
    Output:
        ...
        1. casos, sum of diagnostic cases per day.
        2. estado, summary of patients according to their health status.
        3. dpto_info, summary of data by department or municipality.
        4. atencion, summary of patients by case location.
        5. recu, summary of patients by 'Recuperado' columns info.

    '''
    #Cumulative sum of diagnosed patients
    casos = reporte_datos(csv_data, 'fecha reporte web', 'Casos diagnosticados')
    ...
    atencion = reporte_datos(csv_data, 'Ubicación del caso', 'Número de pacientes')

    #Patients according to medical status
    estado = reporte_datos(csv_data, 'Estado', 'Número de pacientes')

    #Patients according to the column 'Recuperado'
    recu = reporte_datos(csv_data, 'Recuperado', 'Número de pacientes')

    return (casos, estado, dpto_info, atencion, recu)
...
(all_data, all_dptos, all_muni) = load_csv_local_data(file)
...
last_element = a.index[-1]
timestampStr = last_element.strftime("%d - %b, %Y")
...
if (tipo_grafica == 'Recuperados' and 'Recuperado' in e.index) or (tipo_grafica == 'Fallecidos' and 'Fallecido' in e.index) or (tipo_grafica == 'Reportados'):
    ...
    #SEX data
    sex_report = reporte_datos(report_data, 'Sexo', 'Número de pacientes').reset_index()
    ...
    age_data = reporte_datos(age_data, 'Edad', 'Número de pacientes').reset_index()
...
st.markdown("Ultima modificación: *" + last_modified + "*")
st.markdown("Fecha de actualización: *" + timestampStr + "*")
